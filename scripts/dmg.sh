#!/usr/bin/env bash
#
# Package Flowtype as a DMG for people to download.
#
...
ARCH="$(uname -m)"
DMG="$OUT_DIR/Flowtype_${VERSION}_${ARCH}.dmg"
...
if codesign -dv --verbose=2 "$APP" 2>&1 | grep -q adhoc; then
  ...
  echo "         Anyone who downloads this will see \"flowtype is damaged\"." >&2
...
echo
echo "$DMG"
echo "$(du -h "$DMG" | cut -f1) — drag Flowtype.app onto Applications to install"
